# min.py
# This programme allows the user to calculate the minimum value of a set of numbers.
# Author: Orla Woods
...
def column_min(iris: pd.DataFrame, column_name: str) -> float: # column_name is a string, returns a float
...
# Raising errors
    ...
    return iris[column_name].min()

# Using the function to calculate the mean of the specified column (sanity check)
# min_sepal_length = column_min(iris, 'sepal_length') # sanity check 

# Show
# print(f"The minimum sepal length: is {min_sepal_length}cm") # sanity check
