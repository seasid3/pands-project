...
The Iris dataset must first be downloaded for use from the source (see https://archive.ics.uci.edu/dataset/53/iris), using the "download button" which reveals a comma separated values (CSV) plain text output (which I opened in a notebook file). Creating an iris.csv file in the respository, I pasted the Iris CSV database there. The source (https://archive.ics.uci.edu/dataset/53/iris) indicates that the the 35th and 38th samples are incorrect so I corrected these as stated in the iris.csv file (I discovered this when trying to do the first "mean" analysis and went back to fix it and ran import csv again).  
