# max.py
# This programme allows the user to calculate the maximum value of a set of numbers.
# Author: Orla Woods
...
# Define a functions to calculate minimum value in a sepcified column in the iris pandas df 	
def column_max(iris: pd.DataFrame, column_name: str) -> float: # column_name is a string, returns a float
...
# Raising errors
    ...
    return iris[column_name].max()

# Using the function to calculate the mean of the specified column (sanity check)
# max_sepal_length = column_max(iris, 'sepal_length') # sanity check 

# Show
# print(f"The maximum sepal length: is {max_sepal_length}cm") # sanity check